...
#define CHAN_TLV_MAX_SIZE (sizeof(struct mwifiex_ie_types_header) + (MWIFIEX_MAX_CHANNELS_PER_SPECIFIC_SCAN * sizeof(struct mwifiex_chan_scan_param_set)))

/* Memory needed to store supported rate */
#define RATE_TLV_MAX_SIZE (sizeof(struct mwifiex_ie_types_rates_param_set) + HOSTCMD_SUPPORTED_RATES)

/* Memory needed to store a max number/size WildCard SSID TLV for a firmware
	scan */
#define WILDCARD_SSID_TLV_MAX_SIZE  \
	(MWIFIEX_MAX_SSID_LIST_LENGTH * \
	 (sizeof(struct mwifiex_ie_types_wildcard_ssid_params) + IEEE80211_MAX_SSID_LEN))

/* Maximum memory needed for a mwifiex_scan_cmd_config with all TLVs at max */
#define MAX_SCAN_CFG_ALLOC (sizeof(struct mwifiex_scan_cmd_config) + sizeof(struct mwifiex_ie_types_num_probes) + sizeof(struct mwifiex_ie_types_htcap) + CHAN_TLV_MAX_SIZE + RATE_TLV_MAX_SIZE + WILDCARD_SSID_TLV_MAX_SIZE)
...
mwifiex_is_rsn_oui_present(struct mwifiex_bssdescriptor *bss_desc, u32 cipher)
{
	...
	if (((bss_desc->bcn_rsn_ie) && ((*(bss_desc->bcn_rsn_ie)).ieee_hdr.element_id == WLAN_EID_RSN)))
	{
		iebody = (struct ie_body *)(((u8 *)bss_desc->bcn_rsn_ie->data) +
									RSN_GTK_OUI_OFFSET);
					  ...
					  struct mwifiex_bssdescriptor *bss_desc)
{
	if (!priv->sec_info.wep_enabled && !priv->sec_info.wpa_enabled &&
		!priv->sec_info.wpa2_enabled && ((!bss_desc->bcn_wpa_ie) || ((*(bss_desc->bcn_wpa_ie)).vend_hdr.element_id != WLAN_EID_WPA)) &&
		((!bss_desc->bcn_rsn_ie) ||
				   ...
				   struct mwifiex_bssdescriptor *bss_desc)
{
	...
	)
	{
		dev_dbg(priv->adapter->dev, "info: %s: WPA:"
				...
				(bss_desc->bcn_rsn_ie) ? (*(bss_desc->bcn_rsn_ie)).ieee_hdr.element_id : 0,
	...
	if (bss_desc->bss_mode == mode)
	{
		...
		else if (mwifiex_is_bss_wpa(priv, bss_desc))
		{
											...
											CIPHER_SUITE_CCMP))
			{
				...
				if (mwifiex_is_wpa_oui_present(bss_desc, CIPHER_SUITE_TKIP))
				{
					dev_dbg(adapter->dev,
		...
		else if (mwifiex_is_bss_wpa2(priv, bss_desc))
		{
											...
											CIPHER_SUITE_CCMP))
			{
				...
				if (mwifiex_is_rsn_oui_present(bss_desc, CIPHER_SUITE_TKIP))
				{
					dev_dbg(adapter->dev,
							"info: Disable 11n if AES "
							"is not supported by AP\n");
		...
		for (i = 0; (i < sband->n_channels); i++)
		{
			...
			if (user_scan_in &&
				...
				scan_chan_list[chan_idx].max_scan_time =
					cpu_to_le16((u16)user_scan_in->chan_list[0].scan_time);
			...
			if (ch->flags & IEEE80211_CHAN_PASSIVE_SCAN)
				scan_chan_list[chan_idx].chan_scan_mode_bitmap |= MWIFIEX_PASSIVE_SCAN;
			else
				scan_chan_list[chan_idx].chan_scan_mode_bitmap &= ~MWIFIEX_PASSIVE_SCAN;
			...
			if (filtered_scan)
			{
				...
				scan_chan_list[chan_idx].chan_scan_mode_bitmap |= MWIFIEX_DISABLE_CHAN_FILT;
			}
			chan_idx++;
		}
	}
	...
	while (tmp_chan_list->chan_number)
	{
			   ...
			   tmp_chan_list->chan_number && !done_early)
		{
			...
			dev_dbg(priv->adapter->dev,
					...
					tmp_chan_list->chan_scan_mode_bitmap & MWIFIEX_PASSIVE_SCAN,
					(tmp_chan_list->chan_scan_mode_bitmap & MWIFIEX_DISABLE_CHAN_FILT) >> 1,
			...
			scan_cfg_out->tlv_buf_len +=
				(sizeof(chan_tlv_out->header) + le16_to_cpu(chan_tlv_out->header.len));
	...
	*scan_current_only = false;
	...
	if (user_scan_in)
	{
		...
		scan_cfg_out->bss_mode =
			(user_scan_in->bss_mode ? (u8)user_scan_in->bss_mode : (u8)adapter->scan_mode);

		/* Set the number of probes to send, use Adapter setting
		   if unset */
		num_probes =
			(user_scan_in->num_probes ? user_scan_in->num_probes : adapter->scan_probes);
		...
		for (i = 0; i < user_scan_in->num_ssids; i++)
		{
			...
			tlv_pos += (sizeof(wildcard_ssid_tlv->header) + le16_to_cpu(wildcard_ssid_tlv->header.len));
	...
	if (user_scan_in && user_scan_in->chan_list[0].chan_number)
	{
			 ...
			 chan_idx++)
		{
			...
			if (scan_type == MWIFIEX_SCAN_TYPE_PASSIVE)
				(scan_chan_list +
				 chan_idx)
					->chan_scan_mode_bitmap |= MWIFIEX_PASSIVE_SCAN;
			else
				(scan_chan_list +
				 chan_idx)
					->chan_scan_mode_bitmap &= ~MWIFIEX_PASSIVE_SCAN;
			...
			{
				scan_dur = (u16)user_scan_in->chan_list[chan_idx].scan_time;
			}
			else
			{
				if (scan_type == MWIFIEX_SCAN_TYPE_PASSIVE)
									...
									u8 *ie_buf, u32 ie_len)
{
	...
	while (bytes_left >= 2)
	{
						...
						sizeof(wpa_oui)))
			{
				bss_entry->bcn_wpa_ie =
					(struct ieee_types_vendor_specific *)
						current_ptr;
				bss_entry->wpa_offset = (u16)(current_ptr - bss_entry->beacon_buf);
			}
		...
		case WLAN_EID_HT_CAPABILITY:
			bss_entry->bcn_ht_cap = (struct ieee80211_ht_cap *)(current_ptr +
		...
		case WLAN_EID_HT_INFORMATION:
			bss_entry->bcn_ht_info = (struct ieee80211_ht_info *)(current_ptr +
																  sizeof(struct ieee_types_header));
							...
							struct mwifiex_scan_cmd_config *scan_cfg)
{
	...
	cmd->size = cpu_to_le16((u16)(sizeof(scan_cmd->bss_mode) + sizeof(scan_cmd->bssid) + scan_cfg->tlv_buf_len + S_DS_GEN));
							...
							struct host_cmd_ds_command *resp)
{
	...
	is_bgscan_resp = (le16_to_cpu(resp->command) == HostCmd_CMD_802_11_BG_SCAN_QUERY);
	if (is_bgscan_resp)
	 ...
	 */
	tlv_buf_size = scan_resp_size - (bytes_left + sizeof(scan_rsp->bss_descript_size) + sizeof(scan_rsp->number_of_sets) + S_DS_GEN);

	tlv_data = (struct mwifiex_ie_types_data *)(scan_rsp->bss_desc_and_tlv_buffer +
												bytes_left);
	...
	mwifiex_ret_802_11_scan_get_tlv_ptrs(adapter, tlv_data, tlv_buf_size,
										 ...
										 (struct mwifiex_ie_types_data **)&tsf_tlv);

	/* Search the TLV buffer space in the scan response for any valid
	   TLVs */
	mwifiex_ret_802_11_scan_get_tlv_ptrs(adapter, tlv_data, tlv_buf_size,
										 TLV_TYPE_CHANNELBANDLIST,
										 (struct mwifiex_ie_types_data **)&chan_band_tlv);
		...
		if (channel)
		{
			...
			if (chan_band_tlv)
			{
				...
				band = mwifiex_radio_type_to_band(
					chan_band->radio_type & (BIT(0) | BIT(1)));
			}
				...
				if (priv->media_connected &&
					!memcmp(bssid,
							priv->curr_bss_params.bss_descriptor
								.mac_address,
							ETH_ALEN))
					mwifiex_update_curr_bss_params(priv, bssid, rssi,
												   ie_buf, ie_len,
												   beacon_period,
								...
								u16 vsie_mask, u8 **buffer)
{
	...
	for (id = 0; id < MWIFIEX_MAX_VSIE_NUM; id++)
	{
		...
		{
			vs_param_set =
				(struct mwifiex_ie_types_vendor_param_set *)*buffer;
			vs_param_set->header.type =
				cpu_to_le16(TLV_TYPE_PASSTHROUGH);
			vs_param_set->header.len =
				cpu_to_le16((((u16)priv->vs_ie[id].ie[1]) & 0x00FF) + 2);
...
void mwifiex_save_curr_bcn(struct mwifiex_private *priv)
{
	...
	if (curr_bss->bcn_wpa_ie)
		curr_bss->bcn_wpa_ie =
			(struct ieee_types_vendor_specific *)(curr_bss->beacon_buf +
												  curr_bss->wpa_offset);

	if (curr_bss->bcn_rsn_ie)
		curr_bss->bcn_rsn_ie = (struct ieee_types_generic *)(curr_bss->beacon_buf +
															 curr_bss->rsn_offset);

	if (curr_bss->bcn_ht_cap)
		curr_bss->bcn_ht_cap = (struct ieee80211_ht_cap *)(curr_bss->beacon_buf +
														   curr_bss->ht_cap_offset);

	if (curr_bss->bcn_ht_info)
		curr_bss->bcn_ht_info = (struct ieee80211_ht_info *)(curr_bss->beacon_buf +
