							...
							struct uvc_buffer *buf)
{
	...
	y = (u64)(y2 - y1) * (1ULL << 31) + (u64)y1 * (u64)x2 - (u64)y2 * (u64)x1;
	y = div_u64(y, x2 - x1);
							...
							size_t size)
{
	...
	if (duration != 0)
		scr_sof_freq = stream->stats.stream.scr_sof_count * 1000 / duration;
	else
		scr_sof_freq = 0;
								   ...
								   struct uvc_buffer *buf, __u8 *data, int len)
{
	...
	data[1] = UVC_STREAM_EOH | UVC_STREAM_EOF | (stream->last_fid & UVC_STREAM_FID);
	return 2;
}
