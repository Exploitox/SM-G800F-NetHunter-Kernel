...
dccp_feat_entry_new(struct list_head *head, u8 feat, bool local)
{
	...
	list_for_each_entry(entry, head, node) if (entry->feat_num == feat && entry->is_local == local)
	{
		dccp_feat_val_destructor(entry->feat_num, &entry->val);
		return entry;
	}
...
int dccp_feat_finalise_settings(struct dccp_sock *dp)
{
	 ...
	 */
	list_for_each_entry(entry, fn, node) if (entry->feat_num == DCCPF_CCID && entry->val.sp.len == 1)
...
int dccp_feat_activate_values(struct sock *sk, struct list_head *fn_list)
{
	...
	list_for_each_entry_safe(cur, next, fn_list, node) if (!cur->needs_confirm)
