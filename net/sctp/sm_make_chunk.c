...
struct sctp_chunk *sctp_make_sack(const struct sctp_association *asoc)
{
	...
	len = sizeof(sack) + sizeof(struct sctp_gap_ack_block) * num_gabs + sizeof(__u32) * num_dup_tsns;
	...
	const struct sctp_chunk *chunk, __u32 tsn)
{
	...
	retval = sctp_make_abort(asoc, chunk, sizeof(sctp_errhdr_t) + sizeof(tsn));
											 ...
											 const __u8 *raw_addrs, int addrs_len)
{
	...
	bodysize = sizeof(struct sctp_cookie) + ntohs(init_chunk->chunk_hdr->length) + addrs_len;
